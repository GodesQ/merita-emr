...
class SOAController extends Controller {
    ...
    public function soa_print(Request $request) {
        ...
        $patients = Admission::whereDate('trans_date', '>=', $request->date_from)->whereDate('trans_date', '<=', $request->date_to)
                ...
                ->where(function ($q) use ($bahia_vessel, $agency_id) {
                    if($agency_id == 3) {
                        ...
                        if($bahia_vessel == 'BLUETERN') {
                            return $q->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BLUE TERN'))
                                ->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BLUETERN'));
                        } 
                        
                        if($bahia_vessel == 'BOLDTERN') {
                            return $q->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BOLD TERN'))
                                ->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BOLDTERN'));
                        } 
                        
                        if($bahia_vessel == 'BRAVETERN') {
                            return $q->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BRAVE TERN'))
                                ->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BRAVETERN'));
                        }
                        
                        if($bahia_vessel == 'BALMORAL') {
                            return $q->orWhere(DB::raw('upper(vesselname)'),  strtoupper('MS BALMORAL'))
                                ->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BALMORAL'));
                        }
                        
                        if($bahia_vessel == 'BOREALIS') {
                            return $q->orWhere(DB::raw('upper(vesselname)'),  strtoupper('MS BOREALIS'))
                                ->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BOREALIS'));
                        }
                        
                        if($bahia_vessel == 'BOLETTE') {
                            return $q->orWhere(DB::raw('upper(vesselname)'),  strtoupper('MS BOLETTE'))
                                ->orWhere(DB::raw('upper(vesselname)'),  strtoupper('BOLETTE'));
                        }
                        
                        if($bahia_vessel == 'BRAEMAR') {
                            return $q->orWhere(DB::raw('upper(vesselname)'),  strtoupper('MS BRAEMAR'))
        ...
        } else if($agency->id == 9 && $request->package_id == 1) {
            $collected_patients = collect($patients)->chunk(10);
        } else if($agency->id == 3 && $request->package_id == 3) {
