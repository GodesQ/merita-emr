...
class ECGController extends Controller
{
    ...
    {   
        try {
            ...
            if ($save) {
                return back()->with('status', 'ECG updated!!');
            }
        } catch (\Exception $exception) {
            $message = $exception->getMessage();
            $file = $exception->getFile();
    ...
    {   
        try {
            ...
            $path =
                ...
                $request->patientcode;
            if ($save) {
                return redirect($path)->with('status', 'ECG added')->with('redirect', 'basic-exam;child-basic-tab;child-basic-component;baseVerticalLeft1-tab5;tabVerticalLeft5');
            }
        } catch (\Exception $exception) {
            $message = $exception->getMessage();
            $file = $exception->getFile();
