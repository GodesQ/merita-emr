...
class PackageController extends Controller
{
  ...
  public function store_package(Request $request)
  {
    ...
    if ($request->has('exams') && count($request->exams) > 0) {
    ...
    if ($request->has('requests') && count($request->requests) > 0) {
  ...
  public function update_package(Request $request)
  {
    if ($request->filled('exams')) {
      DB::table('list_packagedtl')
        ->where('main_id', $request->id)
        ->delete();

      $data = collect($request->exams)->map(fn($exam) => [
        'main_id' => $request->id,
        'exam_id' => $exam,
      ])->toArray();

      DB::table('list_packagedtl')->insert($data);
    }
