<div class="content-body">
    <form action="/update_cashier_or" id="update_cashier_or" method="POST">
        @csrf
        <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-6 col-xs-12">
                                        <div class="form-group">
                                            ...
                                            <button type="button" class="btn btn-outline-primary my-50 {{$patient_or->payment_user == 'applicant' ? 'active' : null}}" id="bill_to_applicant" onclick="updatePayment('applicant_paid')">APPLICANT</button>
                                            <button type="button" class="btn btn-outline-primary my-50 {{$patient_or->payment_user == 'agency' ? 'active' : null}}" id="bill_to_agency" onclick="updatePayment('package')">AGENCY</button>
                                        </div>
                                    </div>
                                    <div class="col-6 col-xs-12">
                                        <label>Payor</label>
                                        <input type="text" class="form-control" name="payor" id="payor" value="{{$patient_or->payor}}" placeholder="Name" readonly>
                                    </div>
                                </div>
                                <div class="row">
                                            ...
                                            <div class="card-content collapse show">
                                                <div class="card-body">
                                                    ...
                                                    <div class="d-inline-block custom-control custom-radio mr-1 package-con">
                                                        <input required type="radio" onchange="selectPaymentType(this)" {{$patient_or->paying_type == 'package' ? 'checked' : null}} value="package" class="custom-control-input" name="payment_type" id="radio1">
                                                        <label class="custom-control-label" for="radio1">package</label>
                                                    </div>
                                                    <div class="d-inline-block custom-control custom-radio mr-1">
                                                        <input required type="radio" onchange="selectPaymentType(this)" {{$patient_or->paying_type == 'exams' ? 'checked' : null}} value="exams" class="custom-control-input" name="payment_type" id="radio2">
                                                        <label class="custom-control-label" for="radio2">exams</label>
                                                    </div>
                                                    <div class="d-inline-block custom-control custom-radio mr-1">
                                                        <input required type="radio" onchange="selectPaymentType(this)" {{$patient_or->paying_type == 'medicine' ? 'checked' : null}} value="medicine" class="custom-control-input" name="payment_type" id="radio3">
                                                        <label class="custom-control-label" for="radio3">medicine</label>
                                                    </div>
                                                    <div class="d-inline-block custom-control custom-radio mr-1">
                                                        <input required type="radio" onchange="selectPaymentType(this)" {{$patient_or->paying_type == 'other' ? 'checked' : null}} value="other" class="custom-control-input" name="payment_type" id="radio4">
                                ...
                                <div class="invoice-product-details">
                                    <div class="container-fluid m-1 p-1 border {{$patient->patientinfo->payment_type == 'Billed' ? 'package' : 'applicant_paid'}}">
                                        ...
                                        <div class="row border p-1">
                                            ...
                                            <div class="col-md-3 text-center">₱ {{$patient_package ? $patient_package->peso_price : null}} @php $total_package += $patient_package ? $patient_package->peso_price : null @endphp</div>
                                    ...
                                    <div class="container-fluid m-1 p-1 border">
                                        ...
                                        @forelse($exam_groups as $exam_date => $exam_group)
                                            <div class="row">
                                                <h4 class="font-weight-bold">{{date_format(new DateTime($exam_date), "F d, Y")}}</h4>
                                    ...
                                    <div class="row">
                                        <div class="col-12 col-md-6 col-lg-6 col-xl-12">
                                            <div class="regarding-discount form-group">
                                                ...
                                                <textarea required type="text" class="form-control" id="particulars" name="particulars" col="8" row="8">{{$patient_or->particulars}}</textarea>
                                            </div>
                                            <div class="regarding-discount form-group">
                                                <label>TIN</label>
                                                <input type="number" class="form-control" name="tin_no" id="tin_no" value="{{$patient_or->tin_no}}" placeholder="Ex.12345">
                                            </div>
                                            <div class="regarding-discount form-group">
                                                <label>Subtotal</label>
                                                <input required type="text" class="form-control" oninput="setSubTotal(this)" name="amount_due" id="sub_total" value="{{$patient_or->amount_due}}" placeholder="">
                                            </div>
                                            <div class="regarding-payment form-group">
                                                <label>Senior(10%)</label>
                                                <input type="number" class="form-control" name="discount" id="discount" value="{{$patient_or->discount}}" placeholder="Add Discount">
                                            </div>
                                            <div class="regarding-payment form-group">
                                                <label>Payment Status</label> <br>
                                                <div class="d-inline-block custom-control custom-radio mr-1 package-con">
                                                    <input required type="radio" value="paid" class="custom-control-input" {{$patient_or->status == 'paid' ? 'checked' : null}} name="status" id="status1">
                                                    <label class="custom-control-label" for="status1">Paid</label>
                                                </div>
                                                <div class="d-inline-block custom-control custom-radio mr-1 package-con">
                                                    <input required type="radio" value="partial_paid" class="custom-control-input" {{$patient_or->status == 'partial_paid' ? 'checked' : null}} name="status" id="status3">
                                                    <label class="custom-control-label" for="status3">Partially Paid</label>
                                                </div>
                                                <div class="d-inline-block custom-control custom-radio mr-1">
                                                    <input required type="radio" value="unpaid" class="custom-control-input" {{$patient_or->status == 'unpaid' ? 'checked' : null}} name="status" id="status2">
                                        ...
                                        <div class="col-12 col-md-6 col-lg-6 col-xl-12">
                                            <ul class="list-group cost-list">
                                                <li class="list-group-item each-cost border-0 p-50 d-flex justify-content-between">
                                                    ...
                                                    <span class="cost-value">₱ <span id="total-sub">{{$patient_or->amount_due}}</span></span>
                                                </li>
                                                <li class="list-group-item each-cost border-0 p-50 d-flex justify-content-between">
                                                    <span class="cost-title mr-2">Discount </span>
                                                    <span class="cost-value">₱ <span id="total-discount">{{$patient_or->discount}}</span></span>
                                                </li>
                                                <li class="dropdown-divider"></li>
                                                <li class="list-group-item each-cost border-0 p-50 d-flex justify-content-between">
                                                    <span class="cost-title mr-2">Total Amount</span>
                                                    <span class="cost-value">₱ <span id="total-amount">{{$patient_or->amount}}</span></span>
                                                    <input type="hidden" name="amount" id="amount">
                                                </li>
                                            </ul>
                                            <input type="hidden" name="payment_user" id="payment_user" value="{{$patient_or->payment_user}}" />
                                            <input type="hidden" name="agency_id" value="{{$patientInfo->agency_id}}" />
                                            <input type="hidden" name="id" value="{{$patient_or->id}}" />
                                            ...
                                            <a href="cashier_or_print?id={{$patient_or->id}}" target="_blank" class="btn btn-primary mt-1 btn-block">Print</a>
                                        </div>
                                    </div>
                                </div>  
                            </div>
...
<script type="text/javascript">
    ...
    window.addEventListener('load', () => {
        updatePayment('{{$patient_or->payment_user == "agency" ? "package" : "applicant_paid"}}');    
    })
    ...
    function updatePayment(e) {
            ...
            const radioButtons = document.querySelectorAll('#update_cashier_or input[name="payment_type"]');
