            ...
            <table width="100%" cellpadding="0" cellspacing="0" class="pageTable" >
                <tbody>
                        ...
                        <td >
                            <table width="100%" cellspacing="0" cellpadding="2" style="margin: 2rem 0 0 0;">
                                <tbody>
                                    ...
                                    <tr>
                                        <td width="20%" style="font-size: 10px; text-decoration: underline;text-transform: uppercase;">{{ date_format(new DateTime($date_from), 'd') }} - {{ date_format(new DateTime($date_to), 'd') }} {{ date_format(new DateTime($date_to), 'F Y') }}</td>
                                        <td width="20%">INVOICE: <span style="border: none; border-bottom: 1px solid black;">{{ $invoice_number }}-{{ $page }}</span></td>
                                    </tr>
                                    <tr>
                    ...
                    <tr>
                        <td>
                            <div>
                                <table width="100%" cellpadding="2" cellspacing="0" class="brdBtm soa-table" border="0" style="margin-top: 1rem;">
                                    <tbody>
                                        <tr>
                                            <td colspan="10" align="center" style="font-size: 15px; font-weight: 500; border: none !important;">STATEMENT OF ACCOUNT</td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td align="left" class="brdTop brdRight brdBtm" width="12%">MIDDLENAME</td>
                                        ...
                                        @forelse($collected_patient as $patient)
                                            @if(in_array('include_package', $include_examination))
                                                <tr>
                                                    ...
                                                    @if(in_array('position', $additional_columns))
                                                        <td valign="top" align="left" class="brdRight">{{$patient->position}}</td>
                                                    @endif
                                                    @if(in_array('age', $additional_columns))
                                                        <td valign="top" align="left" class="brdRight">{{$patient->patient ? $patient->patient->age : null}}</td>
                                                    @endif
                                                    @if(in_array('referral', $additional_columns))
                                                        <td valign="top" align="left" class="brdRight">{{$patient->referral}}</td>
                                                    @endif
                                                    @if(in_array('vessel', $additional_columns))
                                                        <td valign="top" align="left" class="brdRight">{{$patient->vesselname}}</td>
                                                    @endif
                                                    <td valign="top" align="left" class="brdRight"><span style="margin-right: 15px;">{{$patient->package->packagename}}</span> <span>{{ $agency->id == 15 ? 'ECG' : null }}</span></td>
                                            ...
                                            @if(in_array('include_additional_exams', $include_examination))
                                                @forelse($patient->admission_exams as $add_exam)
                                                    <tr>
                                                        <td valign="top" align="left" class="brdLeft brdRight" class="brdRight">{{date_format(new DateTime($add_exam->updated_date), "d-M-Y")}}</td>
                                                        ...
                                                        @if(in_array('vessel', $additional_columns))
                                                            <td valign="top" align="left" class="brdRight"></td>
                                                        @endif
                                                        <td valign="top" align="left" class="brdRight">{{$add_exam->exam ? $add_exam->exam->examname : null}}</td>
                                                        <td valign="top" align="left" class="brdRight">
                                                            <?php
                                                            // To Dollar Exchange
                                        ...
                                        @endforelse
                                        <tr>
                                            <td class="brdTop brdLeft brdRight" colspan="{{ 7 + count($additional_columns) }}">
                                                @if($exchange_rate)
                                                    Bangko Sentral ng Pilipinas (BSP) Reference Exchange Rate {{ $soa_date }} :
                                                    1$ = PHP {{$exchange_rate}} 
                                                @endif
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            @if($total_ecg > 0) 
                                                ...
                                                ?>
                                            @endif
                                            <td class="brdTop brdLeft" colspan="{{ 5 + count($additional_columns) }}">
                                            ...
                                            <td class="brdTop brdRight" colspan="1">
                                                ...
                                                <div style="display: flex; justify-content: space-between; align-items: center;">
                                                    <div style="width: {{ $total_ecg > 0 ? '50%' : '0%' }} !important;">
                                                        @if($total_ecg)
                                                            <div>{{ $currency == 'Pesos' ? '₱' : '' }} <?php echo number_format((float)$sub_amount_ecg, 2); ?></div>
                                                            <div style="margin: 5px 0;">({{ $currency == 'Pesos' ? '₱' : '' }} <?php echo number_format((float)$total_tax_amount_ecg, 2); ?>)</div>
                                                            <hr>
                                                            {{ $currency == 'Pesos' ? '₱' : '$' }} <?php echo number_format((float)$total_amount_ecg, 2); ?>
                                                        @endif
                                                    </div>
                                                    <div style="width: {{ $total_ecg > 0 ? '50%' : '100%' }} !important;">
                                                        @if($tax)
                                                            <div>{{ $currency == 'Pesos' ? '₱' : '' }} <?php echo number_format((float)$sub_amount, 2); ?></div>
                                                            <div style="margin: 5px 0;">({{ $currency == 'Pesos' ? '₱' : '' }} <?php echo number_format((float)$total_tax_amount, 2); ?>)</div>
                                                            <hr>
                    ...
                    <tr>
                        <td>
                            <table width="100%" cellpadding="3" cellspacing="0">
                                <tbody>
                                    <tr>
                                        <td> All statement of account with unpaid balances of more than two months on the agreed due date, a two(2) % monthly interest shall apply. </td>
                                    </tr>
                                    <tr>
                        ...
                        <td>
                            <table width="100%" cellpadding="6" cellspacing="6" style="margin-top: 2rem;">
                                <tbody>
                                    <tr>
                                        <td>
                                            Prepared by / Checked by: <span style="border-bottom: 1px solid black;"><input style="border: none; border-bottom: 1px solid black;" value="{{ $prepared_by }}" /> </span>
                                        </td>
                                        <td>
                                            Approved by: <span style="border-bottom: 1px solid black;"><input style="border: none; border-bottom: 1px solid black;" value="{{ $approved_by }}" /></span>
                                        </td>
                                        <td>Received by: <span style="border-bottom: 1px solid black;"><input style="border: none; border-bottom: 1px solid black;" value=""/></span>
                                        </td>
                                    </tr>
                                    <tr >
                                        <td>Form No. 60 | Rev. 00/20-02-2018</td>
                                        <td style="position: absolute; bottom: 20px; left: 45%;">Page <?php echo $page; $page++ ?> of <?php echo count($collected_patients) ?></td>
